...
def get_hybrid_questions(level_choice):
    try:
        ...
        # FIX: Force everything to lowercase to match your Excel 'beginner/intermediate/advanced'
        df['Level'] = df['Level'].astype(str).str.strip().str.lower()
        target = str(level_choice).strip().lower()
        
        tech_filtered = df[df['Level'] == target]
        
        if tech_filtered.empty:
            st.error(f"Excel Match Failed! Looking for '{target}', but found: {df['Level'].unique()}")
            return pd.DataFrame()
...
if not st.session_state.started:
    with st.expander("Admin: Set Level"):
        ...
        if key == ADMIN_MASTER_KEY:
            # Match these exactly to your Excel lowercase levels
    ...
    if st.button("Start Assessment"):
        if name and len(contact) == 10:
            # These keys match your Apps Script 'data.name', 'data.dob', etc.
...
# --- ASSESSMENT & SUBMISSION ---
    ...
    if st.session_state.review_mode:
        if st.button("Final Submit", type="primary"):
            ...
            final_score = f"{round((correct/25)*100, 2)}%"
            duration = f"{round((time.time() - st.session_state.start_time)/60, 2)} mins"
            
            # This payload matches your 11 columns in Apps Script
            payload = {**st.session_state.candidate_data, "score": final_score, "duration": duration, "answers": json.dumps(st.session_state.answers)}
            
            try:
                res = requests.post(BRIDGE_URL, json=payload, timeout=15)
                if res.status_code == 200:
                    st.balloons(); st.success("Submitted!"); st.session_state.started = False
        ...
        ans = st.radio("Select:", [q_row["Option A"], q_row["Option B"], q_row["Option C"], q_row["Option D"]], key=f"q{idx}")
        if st.button("Next"):
