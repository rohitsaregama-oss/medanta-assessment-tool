...
# --- CONFIGURATION ---
BRIDGE_URL = "https://script.google.com/macros/s/AKfycbxg9Wv4DZIEJGqodXMjxvfLqeGGhQtOmHQ2RHuCFOxzCxzCr-_uz2rOTfeY0lp4G4yT/exec"
...
# --- HEADER WITH LOGOS ---
col_logo1, col_space, col_logo2 = st.columns([2, 4, 2])
try:
    logo1 = Image.open("MHPL logo 2.png")
    logo2 = Image.open("Assessment_tool_logo.png")
    col_logo1.image(logo1, use_container_width=True)
    col_logo2.image(logo2, use_container_width=True)
except:
...
def get_hybrid_questions(level_choice):
    try:
        ...
        tech_filtered = df[df['Level'].astype(str).str.lower() == str(level_choice).lower()]
...
if not st.session_state.started:
    with st.expander("Admin: Set Assessment Level"):
    ...
    if st.button("Start Assessment"):
        if name and len(contact) == 10 and contact.isdigit():
            # Correctly store in session state to avoid NameError later
            st.session_state.candidate_data = {
                "name": name, "dob": str(dob), "qualification": qual, 
                "category": cat, "reg_no": reg, "college": college, "contact": contact,
                "assigned_level": st.session_state.level
            }
...
# --- STEP 2: ASSESSMENT UI ---
    ...
    st.metric("Total Time Remaining", f"{int(rem_total // 60)}m {int(rem_total % 60)}s")

    if st.session_state.review_mode:
        st.subheader("Review Your Answers")
        for i in range(len(q_df)):
            c1, c2, c3 = st.columns([5, 3, 2])
            c1.write(f"Q{i+1}: {q_df.iloc[i]['question'][:40]}...")
            c2.write(f"**{st.session_state.answers.get(f'Q{i+1}', 'No Answer')}**")
            if c3.button("Edit", key=f"rev_ed_{i}"):
        ...
        if st.button("Final Submit", type="primary") or rem_total <= 0:
            ...
            correct_count = 0
            for i in range(len(q_df)):
                user_ans = st.session_state.answers.get(f"Q{i+1}")
                actual_ans = q_df.iloc[i]["Correct Answer"]
                if str(user_ans).strip() == str(actual_ans).strip():
                    correct_count += 1
            
            final_score = f"{round((correct_count / len(q_df)) * 100, 2)}%"
            duration = f"{round((time.time() - st.session_state.start_time) / 60, 2)} mins"
            
            # Combine data from session state
            payload = {
                **st.session_state.candidate_data,
                "score": final_score,
                "duration": duration,
            ...
            try:
                # Optimized post with headers
                res = requests.post(BRIDGE_URL, json=payload, timeout=15)
                if res.status_code == 200:
                    st.balloons()
                    st.success(f"Submitted! Your Score: {final_score}")
                    st.session_state.started = False 
                else:
                    st.error("Bridge Error. Check Deployment.")
            except:
                st.error("Connection Error.")

    else:
        # Standard question display
        rem_q = max(0, QUESTION_TIME_LIMIT - (now - st.session_state.q_start_time))
        st.metric("Question Timer", f"{int(rem_q)}s")
        if rem_q <= 0:
            st.session_state.answers[f"Q{st.session_state.q_index+1}"] = "TIMEOUT"
            if st.session_state.q_index < 24:
                st.session_state.q_index += 1; st.session_state.q_start_time = time.time(); st.rerun()
            else: st.session_state.review_mode = True; st.rerun()

        q_idx = st.session_state.q_index
        q_row = q_df.iloc[q_idx]
        st.markdown(f"### Q{q_idx + 1} of 25")
        st.write(f"**{q_row['question']}**")
        opts = [q_row["Option A"], q_row["Option B"], q_row["Option C"], q_row["Option D"]]
        current_ans = st.session_state.answers.get(f"Q{q_idx+1}", opts[0])
        choice = st.radio("Answer:", opts, key=f"rad_{q_idx}", index=opts.index(current_ans) if current_ans in opts else 0)

        c_prev, c_next = st.columns(2)
        if q_idx > 0 and c_prev.button("Previous"):
            st.session_state.q_index -= 1; st.session_state.q_start_time = time.time(); st.rerun()
        if c_next.button("Next" if q_idx < 24 else "Review All"):
            st.session_state.answers[f"Q{q_idx+1}"] = choice
            if q_idx < 24:
                st.session_state.q_index += 1; st.session_state.q_start_time = time.time(); st.rerun()
            else: st.session_state.review_mode = True; st.rerun()