...
if not st.session_state.started:
    ...
    if st.button("Start Assessment"):
        if not name or not mobile or reg_auth == "Choose an option...":
            st.warning("Please fill Name, Mobile, and Select Registration Authority.")
        else:
            try:
                ...
                st.session_state.candidate = {
                    ...
                    "reg_no": reg_no, "reg_auth": reg_auth, "level": st.session_state.level
                }
...
# ================= EXAM =================
    ...
    st.markdown(f"<div class='timer-box'>⏱ Time Remaining: {remain}s</div>", unsafe_allow_html=True)
    ...
    choice = st.radio("Options:", [q["option_a"], q["option_b"], q["option_c"], q["option_d"]], key=f"q{st.session_state.idx}")
...
# ================= RESULTS (Correct Column Mapping) =================
else:
    correct = 0
    payload = {**st.session_state.candidate}
    
    # Strictly mapping 25 questions as Q1, Q2, etc., and Q1_status for coloring
    for i in range(25):
    ...
    st.markdown(f"<div class='card' style='text-align:center;'><h2>Score: {correct}/25</h2><p>Bridge Key: {st.session_state.candidate['bridge_key']}</p></div>", unsafe_allow_html=True)
    ...
    if not st.session_state.submitted:
        try:
            ...
            st.error("Submission failed. Please screenshot this score.")
